...
export function renderTemplate(templatePath: string, data: PageData): string {
  // Baca layout JSON
  const layoutFile = path.join(__dirname, 'templates', templatePath, 'layout.json');
...
`;
  ...
  for (const section of layout.sections) {
    if (section.type === 'hero') {
      ...
      const items = data.features || section.props.defaultItems;
