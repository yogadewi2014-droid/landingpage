...
export async function render(data: LandingPageData): Promise<string> {
  ...
  if (!process.env.PEXELS_API_KEY) {
    console.warn("[restaurant.ts] PEXELS_API_KEY kosong. Gambar dari Pexels tidak tersedia.");
  }
  if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_UPLOAD_PRESET) {
    console.warn("[restaurant.ts] CLOUDINARY config kurang. Upload ke Cloudinary dilewati.");
  }
  ...
  const subheadline = escapeHtml(data.subheadline || "Pengalaman kuliner premium dengan cita rasa istimewa.");
  ...
  const ctaDescription = escapeHtml(data.ctaDescription || "Nikmati pengalaman makan premium bersama orang tercinta.");
