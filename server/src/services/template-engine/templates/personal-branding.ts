...
export async function render(data: LandingPageData): Promise<string> {
  ...
  if (!process.env.PEXELS_API_KEY) {
    console.warn("[personal.ts] PEXELS_API_KEY kosong. Gambar dari Pexels tidak tersedia.");
  }
  if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_UPLOAD_PRESET) {
    console.warn("[personal.ts] CLOUDINARY config kurang. Upload ke Cloudinary dilewati.");
  }
  ...
  const subheadline = escapeHtml(data.subheadline || "Tampilkan diri Anda dengan elegan dan profesional.");
  ...
  const ctaDescription = escapeHtml(data.ctaDescription || "Bangun koneksi dan peluang baru bersama saya.");
  const ctaText = escapeHtml(data.cta || "Hubungi Saya");
  const ctaLink = escapeHtml(data.ctaLink || "#");

  // Sanitasi link sosial media
  const socialInstagram = data.socialInstagram ? escapeHtml(data.socialInstagram) : "";
  const socialWhatsapp = data.socialWhatsapp ? escapeHtml(data.socialWhatsapp) : "";
  const socialTiktok = data.socialTiktok ? escapeHtml(data.socialTiktok) : "";
  ...
  // Susun HTML link sosial media
  const socialsHtml = [];
  if (socialInstagram) {
    socialsHtml.push(`<a href="${socialInstagram}">Instagram</a>`);
  }
  if (socialWhatsapp) {
    socialsHtml.push(`<a href="${socialWhatsapp}">WhatsApp</a>`);
  }
  if (socialTiktok) {
    socialsHtml.push(`<a href="${socialTiktok}">TikTok</a>`);
  }
